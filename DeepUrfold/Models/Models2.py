...
#       kernel_size=3, downsample=[2,2], nPlanes=None):
#         nn.Module.__init__(self)
#         if nPlanes is None:
#             nPlanes = [l*m for l in range(1,nLevels+1)]
#         self.sparseModel = scn.Sequential().add(
#            scn.InputLayer(dimension, torch.LongTensor([sz]*3), mode=3)).add(
#            scn.SubmanifoldConvolution(dimension, nIn, m, kernel_size, False)).add(
#            UNet(dimension, reps, nPlanes, residual_blocks=True, downsample=downsample,
#                kernel_size=kernel_size)).add(
...
class SegmenterAutoEncoderModel(nn.Module):
    ...
    nPlanes = [m, 2*m, 3*m, 4*m, 5*m] #UNet number of features per level
    """
    def __init__(self, nIn, nOut=2, sz=256, dimension=3, reps=1, m=32, nLevels=5,
      kernel_size=3, nPlanes=None):
        nn.Module.__init__(self)
        if nPlanes is None:
            nPlanes = [l*m for l in range(1,nLevels+1)]
        ...
        self.sparseModel = scn.Sequential().add(
           scn.InputLayer(dimension, torch.LongTensor([sz]*3), mode=3)).add(
           scn.SubmanifoldConvolution(dimension, nIn, m, kernel_size, False)).add(
           UNet(dimension, reps, nPlanes, residual_blocks=True, downsample=[2,2],
              kernel_size=kernel_size)).add(
           scn.BatchNormReLU(m)).add(
           scn.OutputLayer(dimension))
...
def UNet(dimension, reps, nPlanes, residual_blocks=False, downsample=[2, 2],
  leakiness=0, n_input_planes=-1, dropout=0.5, kernel_size=3, skip_connections=True):
    """
...
def VariationalUNetAutoEncoder(dimension, reps, nPlanes, residual_blocks=False,
  downsample=[2, 2], leakiness=0, n_input_planes=-1, dropout=0.5, kernel_size=3, nLatent=2):
    """
    U-Net style network with VGG or ResNet-style blocks.
    For voxel level prediction:
    import sparseconvnet as scn
