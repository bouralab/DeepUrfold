...
from DeepUrfold.Models.minkvae import Encoder, Decoder
from DeepUrfold.util import str2bool

class DomainStructureVAE(pl.LightningModule):
    """
    Standard VAE with Gaussian Prior and approx posterior.
    """
    LATENT_VARS = ["mean", "log_var"]
    ...
    def forward(self, x):
        ...
        z = means
        if self.training or self.hparams.sample_during_eval:
            z = self.sample(means, log_vars)
            #p, q, z = self.sample(mu, log_var)
        sout = self.decoder(z) #, target_key)
        return z, sout, means, log_vars #out_cls, targets, sout, mu, log_var, z

    def sample(self, means, log_vars):
        # std = torch.exp(log_var.F / 2)
        # p = torch.distributions.Normal(torch.zeros_like(mu.F), torch.ones_like(std))
