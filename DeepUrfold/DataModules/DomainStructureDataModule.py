...
class DomainStructureDataModule(pl.LightningDataModule):
    DATASET = DomainStructureDataset

    def __init__(self, hparams, eval=False):
        assert self.DATASET is not None, "Must subclass and set DATASET"
        super().__init__()

        #Don't use reserved keyword hparams
        self.h_params = hparams
        self.data_dir = self.h_params.data_dir
        self.train_dir = os.path.join(self.data_dir, "train_files")

        #if self.h_params.distributed:
        #    self.DATASET = DistributedDomainStructureDataset

        print("Got features:", self.h_params.features,
            isinstance(self.h_params.features, (tuple, list)),
        ...
        if self.h_params.superfamily is not None:
            ...
            self.train_dir = [os.path.join(self.train_dir, *superfamily.replace("/",".").split(".")) \
                    for superfamily in self.h_params.superfamily]
            # if len(self.h_params.superfamily) == 1:
            #     self.h_params.superfamily = self.h_params.superfamily[0]
        ...
        if not eval:
            if len(self.train_dir) > 1:
                raise RuntimeError("Multiple superfamily training not supported yet.")

            self.train_file = os.path.join(self.train_dir[0], "{}-train-{}-split{:.1f}.h5".format(
                prefix, self.cluster_level, self.h_params.split_size))

            self.valid_file = os.path.join(self.train_dir[0], "{}-valid-{}-split{:.1f}.h5".format(
                prefix, self.cluster_level, (1-self.h_params.split_size)/2))

            self.test_file = os.path.join(self.train_dir[0], "{}-test-{}-split{:.1f}.h5".format(
                prefix, self.cluster_level, (1-self.h_params.split_size)/2))
        elif self.h_params.test_file is not None:
            self.train_file = ""
            self.valid_file = ""
            self.test_file = self.h_params.test_file
        elif self.h_params.representatives:
            self.train_file = ""
            self.valid_file = ""
        ...
        if stage == 'test' or stage is None:
            try:
                self.test_dataset = self.DATASET(
                    ...
                    volume=self.h_params.input_size,
                    rotate=False,
                    test=not (self.h_params.ignore_splits_on_error and self.train_dataset is None and self.valid_dataset is None), #True
                    expand_surface_data_loader=self.h_params.expand_surface_data_loader,
                    compare_kdtree=self.h_params.compare_kdtree,
                ...
                #Selected split is empty bc there is dataset in h5 file
                if not self.h_params.ignore_splits_on_error or (self.train_dataset is None and self.valid_dataset is None):
                    raise
    ...
    def _get_dataloader(self, dataset, workers_split=1, shuffle=True, drop_last=True):
        ...
        batch_size = max(1, min(self.h_params.batch_size, int(len(dataset)/self.h_params.gpus)))
    ...
    def add_data_specific_args(parent_parser, eval=False):
        ...
        data.add_argument("--superfamily", type=str, default=None, nargs='+',
            help="CATH superfamily code. If specified, it will create a superfamily-specific model")
        data.add_argument("--data_dir", default="")
        data.add_argument("--distibuted", type=str2bool, nargs='?',
            const=True, default=False)

        if not eval:
            data.add_argument("--cluster_level", default="H", choices=DomainStructureDataset.hierarchy)
            ...
            data_types.add_argument("--cluster_level", default="H", choices=DomainStructureDataset.hierarchy)
        ...
        data.add_argument("--feature_groups", default=default_feature_features, nargs="+")
