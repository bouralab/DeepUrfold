...
from Prop3D.ml.atasets.DomainStructureDataset import DomainStructureDataset
...
from Prop3D.generate_data.get_cath_representatives import get_representatives
...
class DistributedDomainStructureDataModule(DistributedDomainStructureDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'test' or stage is None:
            ...
            if isinstance(self.test_dataset, self.DATASET):
                self.domain_to_superfamily = dict([reversed(domain.split("/domains/")) for domain in self.test_dataset.order])
            else:
                #It's a ConcatDataset
                self.domain_to_superfamily = dict([reversed(domain.split("/domains/")) for ds in self.test_dataset.order for domain in ds])
    ...
    def _get_dataloader(self, dataset, workers_split=1, shuffle=True, drop_last=True):
        ...
        self.h_params.batch_size = batch_size
