...
class DeepUrfoldAllVsAll(StructureBasedAllVsAll):
    ...
    def __init__(self, superfamilies, data_dir, hparams, latent=False, all=False, classification=None, permutation_dir=None, work_dir=None, force=False, result_dir="", result_dir2=None, downsample_sbm=False, cluster=True):
    ...
    def infer_gpu(self, i, model_name, model_path, combined_dataset, gpu=0):
        ...
        if self.classification == "validation":
            ...
            command = [
                sys.executable,
                "-m", "DeepUrfold.Evaluators.EvaluateDistrubutedDomainStructureVAE",
                "--superfamily", *combined_dataset.superfamily.drop_duplicates().tolist(),
                "--domains", *combined_dataset.cathDomain.tolist(),
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("ava_superfamily", nargs="+", required=False, default=sfams)
    ...
    print(args)
    runner = DeepUrfoldAllVsAll(args.ava_superfamily, args.data_dir, args, latent=args.latent, all=not args.single_model_umap, classification=args.classification, permutation_dir=args.permutation_dir,
        work_dir=args.work_dir, force=args.force, result_dir=args.result_dir, result_dir2=args.result_dir2, downsample_sbm=args.downsample_sbm, cluster=not args.no_cluster)
    runner.run()
